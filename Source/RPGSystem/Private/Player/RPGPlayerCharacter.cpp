...
void ARPGPlayerCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	BindInputActions(RPGInputComp);
}

void ARPGPlayerCharacter::BindInputActions(UCustomInputComponent* RPGInputComp)
{
...
void ARPGPlayerCharacter::ApplyInputPreset(UDataAsset_InputConfig* NewInputConfig, UInputMappingContext* NewMappingContext)
{
	if (!Controller)
	{
		return;
	...
	if (APlayerController* PlayerController = Cast<APlayerController>(Controller))
	{
		if (UEnhancedInputLocalPlayerSubsystem* Subsystem = ULocalPlayer::GetSubsystem<UEnhancedInputLocalPlayerSubsystem>(PlayerController->GetLocalPlayer()))
		{
			...
			{
				DefaultMappingContext = NewMappingContext;
				Subsystem->AddMappingContext(DefaultMappingContext, 0);
			}
		}
	}
	...
	if (UCustomInputComponent* RPGInputComp = Cast<UCustomInputComponent>(InputComponent))
	{
		RPGInputComp->ClearActionBindings();
		BindInputActions(RPGInputComp);
	}
}
